...
if __name__ == "__main__":
    ...
    create_table_sql = str(Path.cwd() / "sql" / "movies_create_table.sql")
    execute_duckdb_cmd(conn, cmd=create_table_sql)

    # Query movies table
    query_table_sql = str(Path.cwd() / "sql" / "movies_query_table.sql")
    result = execute_duckdb_query(conn, query=query_table_sql)

    print("\n Top 250 movies on IMDB / Top 15 Directors\n")
    print(result)
    print("")
